...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  //Oficio
  if ($body['OficioP']<>'') {
  	$RutaOficioP64 = "OficiosYRecibos/".$body['OficioP'];
  	if ($body['OficioP64']<>'') {
  ...
  //Recibo
  if ($body['OficioR']<>'') {
  	$RutaOficioR64 = "OficiosYRecibos/".$body['OficioR'];
  	if ($body['OficioR64']<>'') {
	...
	try {
	  ...
	  $stmt = $con->prepare("UPDATE Oficio SET cla_oficio=:cla_Oficio , nom_oficio=:nom_Oficio , asu_oficio=:asu_oficio , ubi_oficio=:ubi_oficio , notas=:notas , fk_id_departamento=:fkid_departamento , fk_id_estado=:fkid_estado , fk_id_persona=:fkid_persona, DepEnviada=:DepEnvi_Oficio, RutaOficioP = :RutaOficioP, RutaOficioR = :RutaOficioR WHERE Oficio.id_oficio = :id_oficio ;");
	  $stmt->execute(array(
