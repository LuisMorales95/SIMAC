...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  //Oficio
  if ($body['OficioP']<>'') {
  	$RutaOficioP64 = "MemoranYRecibos/".$body['OficioP'];
  	if ($body['OficioP64']<>'') {
  ...
  //Recibo
  if ($body['OficioR']<>'') {
  	$RutaOficioR64 = "MemoranYRecibos/".$body['OficioR'];
  	if ($body['OficioR64']<>'') {
	...
	try {
	  ...
	  $stmt = $con->prepare("UPDATE Memoran SET cla_memoran=:cla_Oficio , nom_memoran=:nom_Oficio , asu_memoran=:asu_oficio , ubi_memoran=:ubi_oficio , notas=:notas , fk_id_departamento=:fkid_departamento , fk_id_estado=:fkid_estado , fk_id_persona=:fkid_persona, DepEnviada=:DepEnvi_Oficio, RutaMemoranP = :RutaOficioP, RutaMemoranR = :RutaOficioR WHERE Memoran.id_memoran = :id_oficio ;");
	  $stmt->execute(array(
