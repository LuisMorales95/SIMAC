...
require 'PHPMailer/src/SMTP.php';
  ...
  try {
    ...
    $cn = new Conexion();
    $con = $cn->Conectar();
    $stmt = $con->prepare('SELECT dbo.MovilCiudadano.id, dbo.MovilCiudadano.Contraseña, dbo.MovilCiudadano.Token, dbo.MovilCiudadano.ImgUrl, dbo.MovilCiudadano.Nombre, dbo.MovilCiudadano.CorreoE, dbo.MovilCiudadano.fk_id_rol, dbo.MovilCiudadano.Usuario_Verificado FROM dbo.MovilCiudadano WHERE Telefono = :telefono;');
    ...
    if ($row[2] != ""||$row[2] != "null") {
      ...
      //echo "<br>";
      $stmt = $con->prepare('SELECT dbo.MovilCiudadano.id, dbo.MovilCiudadano.Contraseña, dbo.MovilCiudadano.Token, dbo.MovilCiudadano.ImgUrl, dbo.MovilCiudadano.Nombre, dbo.MovilCiudadano.ApellidoP, dbo.MovilCiudadano.CorreoE, dbo.MovilCiudadano.fk_id_rol, dbo.MovilCiudadano.Usuario_Verificado FROM dbo.MovilCiudadano WHERE Telefono = :telefono;');
      $stmt->execute(array(':telefono' => $telefono ));
  ...
  function enviarcorreo ($CorreoEs){
...
<br>
    ...
    $mail->addAddress($CorreoEs);     // Add a recipient
